...
export class ShipwreckPieces$ShipwreckPiece extends TemplateStructurePiece {
    ...
    constructor(structureTemplateManager: StructureTemplateManager, templateLocation: Identifier, position: BlockPos, rotation: Rotation, isBeached: boolean)
    // private heightAdjusted: boolean;
    // private isBeached: boolean;
    addAdditionalSaveData(context: StructurePieceSerializationContext, tag: CompoundTag): void;
    adjustPositionHeight(newHeight: number): void;
    calculateBeachedPosition(minY: number, random: RandomSource): number;
