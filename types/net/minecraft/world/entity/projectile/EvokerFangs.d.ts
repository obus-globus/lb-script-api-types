...
export class EvokerFangs extends Entity implements TraceableEntity {
    ...
    constructor(type: EntityType<EvokerFangs>, level: Level)
    constructor(level: Level, x: number, y: number, z: number, rotaionRadians: number, warmupDelayTicks: number, owner: LivingEntity)
    // private clientSideAttackStarted: boolean;
    // private lifeTicks: number;
    readonly owner: EntityReference<LivingEntity>;
    // private sentSpikeEvent: boolean;
    // private warmupDelayTicks: number;
    addAdditionalSaveData(output: ValueOutput): void;
    // private dealDamageTo(entity: LivingEntity): void;
    defineSynchedData(entityData: SynchedEntityData$Builder): void;
    getAnimationProgress(a: number): number;
