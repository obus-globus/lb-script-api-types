...
export class DustParticle$Provider extends Object implements ParticleProvider<DustParticleOptions> {
    constructor(sprites: SpriteSet)
    // private sprites: SpriteSet;
    createParticle(options: DustParticleOptions, level: ClientLevel, x: number, y: number, z: number, xAux: number, yAux: number, zAux: number, random: RandomSource): Particle;
}