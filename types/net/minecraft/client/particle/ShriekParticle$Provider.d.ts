...
export class ShriekParticle$Provider extends Object implements ParticleProvider<ShriekParticleOption> {
    constructor(sprite: SpriteSet)
    // private sprite: SpriteSet;
    createParticle(options: ShriekParticleOption, level: ClientLevel, x: number, y: number, z: number, xAux: number, yAux: number, zAux: number, random: RandomSource): Particle;
}