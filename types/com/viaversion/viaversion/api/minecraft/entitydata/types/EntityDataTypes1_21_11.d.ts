import type { EntityDataType } from '../../../../../../../com/viaversion/viaversion/api/minecraft/entitydata/EntityDataType.d.ts'
import type { AbstractEntityDataTypes } from '../../../../../../../com/viaversion/viaversion/api/minecraft/entitydata/types/AbstractEntityDataTypes.d.ts'
import type { VersionedTypesHolder } from '../../../../../../../com/viaversion/viaversion/api/type/types/version/VersionedTypesHolder.d.ts'
export class EntityDataTypes1_21_11 extends AbstractEntityDataTypes {
    constructor(arg0: VersionedTypesHolder)
    armadilloState: EntityDataType;
    blockPositionType: EntityDataType;
    blockStateType: EntityDataType;
    booleanType: EntityDataType;
    byteType: EntityDataType;
    catVariantType: EntityDataType;
    chickenVariantType: EntityDataType;
    componentType: EntityDataType;
    copperGolemState: EntityDataType;
    cowVariantType: EntityDataType;
    directionType: EntityDataType;
    floatType: EntityDataType;
    frogVariantType: EntityDataType;
    humanoidArmType: EntityDataType;
    itemType: EntityDataType;
    longType: EntityDataType;
    mannequinProfileType: EntityDataType;
    optionalBlockPositionType: EntityDataType;
    optionalBlockStateType: EntityDataType;
    optionalComponentType: EntityDataType;
    optionalGlobalPosition: EntityDataType;
    optionalUUIDType: EntityDataType;
    optionalVarIntType: EntityDataType;
    paintingVariantType: EntityDataType;
    particleType: EntityDataType;
    particlesType: EntityDataType;
    pigVariantType: EntityDataType;
    poseType: EntityDataType;
    quaternionType: EntityDataType;
    rotationsType: EntityDataType;
    snifferState: EntityDataType;
    stringType: EntityDataType;
    varIntType: EntityDataType;
    vector3FType: EntityDataType;
    villagerDataType: EntityDataType;
    weatheringCopperState: EntityDataType;
    wolfSoundVariantType: EntityDataType;
    wolfVariantType: EntityDataType;
    zombieNautilusVariantType: EntityDataType;
}