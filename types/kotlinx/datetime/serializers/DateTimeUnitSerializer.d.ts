import type { KClass } from '../../../kotlin/reflect/KClass.d.ts'
import type { DateTimeUnit } from '../../../kotlinx/datetime/DateTimeUnit.d.ts'
import type { DeserializationStrategy } from '../../../kotlinx/serialization/DeserializationStrategy.d.ts'
import type { SealedClassSerializer } from '../../../kotlinx/serialization/SealedClassSerializer.d.ts'
import type { SerializationStrategy } from '../../../kotlinx/serialization/SerializationStrategy.d.ts'
import type { SerialDescriptor } from '../../../kotlinx/serialization/descriptors/SerialDescriptor.d.ts'
import type { CompositeDecoder } from '../../../kotlinx/serialization/encoding/CompositeDecoder.d.ts'
import type { Encoder } from '../../../kotlinx/serialization/encoding/Encoder.d.ts'
import type { AbstractPolymorphicSerializer } from '../../../kotlinx/serialization/internal/AbstractPolymorphicSerializer.d.ts'
export class DateTimeUnitSerializer extends AbstractPolymorphicSerializer<DateTimeUnit> {
    static INSTANCE: DateTimeUnitSerializer;
    readonly baseClass: KClass<DateTimeUnit>;
    readonly descriptor: SerialDescriptor;
    // private impl: SealedClassSerializer<DateTimeUnit>;
    // private /*not mapped: */ getImpl(): SealedClassSerializer<DateTimeUnit>;
    findPolymorphicSerializerOrNull(decoder: CompositeDecoder, klassName: string | null): DeserializationStrategy<DateTimeUnit> | null;
    findPolymorphicSerializerOrNull(encoder: Encoder, value: DateTimeUnit): SerializationStrategy<DateTimeUnit> | null;
}