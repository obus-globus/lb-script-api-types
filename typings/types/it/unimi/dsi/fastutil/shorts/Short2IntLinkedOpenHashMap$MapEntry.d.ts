import type { Short2IntMap$Entry } from '../../../../../it/unimi/dsi/fastutil/shorts/Short2IntMap$Entry.d.ts'
import type { ShortIntPair } from '../../../../../it/unimi/dsi/fastutil/shorts/ShortIntPair.d.ts'
import type { Comparator } from '../../../../../java/util/Comparator.d.ts'
import type { Object } from '../../../../../java/lang/Object.d.ts'
import type { Map$Entry } from '../../../../../java/util/Map$Entry.d.ts'
export class Short2IntLinkedOpenHashMap$MapEntry extends Object implements Short2IntMap$Entry, ShortIntPair, Map$Entry<number, number> {
    static comparingByKey(): (param0: Object | null) => kotlin.Boolean;
    static comparingByKey(paramarg0: (param0: Object | null) => kotlin.Boolean): (param0: Object | null) => kotlin.Boolean;
    static comparingByValue(): (param0: Object | null) => kotlin.Boolean;
    static comparingByValue(paramarg0: (param0: Object | null) => kotlin.Boolean): (param0: Object | null) => kotlin.Boolean;
    static copyOf(paramarg0: Map$Entry<Object, Object>): Map$Entry<Object, Object>;
    static lexComparator(): (param0: ShortIntPair) => kotlin.Boolean;
    static of(paramarg0: number, paramarg1: number): ShortIntPair;
    constructor(null_: Short2IntLinkedOpenHashMap$MapEntry)
    constructor(null_: Short2IntLinkedOpenHashMap$MapEntry, arg1: number)
    // private index: number;
    readonly key: number;
    readonly value: number;
    equals(arg0: Object | null): boolean;
    first(): number;
    first(arg0: number): ShortIntPair;
    first(arg0: number): ShortIntPair;
    firstShort(): number;
    getIntValue(): number;
    getShortKey(): number;
    hashCode(): number;
    key(): number;
    key(arg0: number): ShortIntPair;
    key(arg0: number): ShortIntPair;
    keyShort(): number;
    left(): number;
    left(arg0: number): ShortIntPair;
    left(arg0: number): ShortIntPair;
    leftShort(): number;
    right(arg0: number): ShortIntPair;
    right(): number;
    right(arg0: number): ShortIntPair;
    right(arg0: number): ShortIntPair;
    rightInt(): number;
    second(): number;
    second(arg0: number): ShortIntPair;
    second(arg0: number): ShortIntPair;
    secondInt(): number;
    setValue(arg0: number): number;
    setValue(arg0: number): number;
    setValue(arg0: number): number;
    toString(): string;
    value(): number;
    value(arg0: number): ShortIntPair;
    value(arg0: number): ShortIntPair;
    valueInt(): number;
}