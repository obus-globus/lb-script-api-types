...
export class ThresholdFilter extends AbstractFilter {
    ...
    static createFilter(paramlevel: Level, parammatch: Filter$Result, parammismatch: Filter$Result): ThresholdFilter;
    private constructor(level: Level, onMatch: Filter$Result, onMismatch: Filter$Result)
    readonly level: Level;
    // private filter(testLevel: Level): Filter$Result;
    filter(event: LogEvent): Filter$Result;
    filter(logger: Logger, testLevel: Level, marker: Marker, msg: Object, t: Throwable): Filter$Result;
    ...
    filter(logger: Logger, testLevel: Level, marker: Marker, msg: string, params: Object[]): Filter$Result;
    filter(logger: Logger, testLevel: Level, marker: Marker, msg: Message, t: Throwable): Filter$Result;
    getLevel(): Level;
    toString(): string;
}