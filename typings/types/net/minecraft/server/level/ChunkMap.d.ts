...
export class ChunkMap extends SimpleRegionStorage implements ChunkMapAccessor, ChunkHolder$PlayerProvider, GeneratingChunkMap {
    ...
    getChunkQueueLevel(pos: number): () => number;
    getChunkRangeFuture(centerChunk: ChunkHolder, range: number, distanceToStatus: (param0: number) => ChunkStatus): CompletableFuture<ChunkResult<ChunkAccess[]>>;
    ...
    // private onLevelChange(pos: ChunkPos, oldLevel: () => number, newLevel: number, setQueueLevel: (param0: number) => void): void;
    ...
    // private processUnloads(haveTime: () => boolean): void;
    ...
    // private saveChunksEagerly(haveTime: () => boolean): void;
    ...
    sendToTrackingPlayersFiltered(entity: Entity, packet: Packet<ClientGamePacketListener>, targetPredicate: (param0: ServerPlayer) => boolean): void;
