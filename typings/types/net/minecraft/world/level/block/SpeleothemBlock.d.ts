...
import type { Fallable } from '../../../../../net/minecraft/world/level/block/Fallable.d.ts'
...
export abstract class SpeleothemBlock extends Block implements Fallable, SimpleWaterloggedBlock {
    static BLOCK_STATE_REGISTRY: BlockState[];
    static CODEC: MapCodec<Block>;
    static FILTERED_REGISTRIES: ResourceKey<FeatureElement[]>[];
    static INDESTRUCTIBLE: number;
    static INSTANT: number;
    static THICKNESS: EnumProperty<SpeleothemThickness>;
    static TIP_DIRECTION: EnumProperty<Direction>;
    ...
    constructor(blockToGrowOn: BlockState, properties: BlockBehaviour$Properties)
    // private blockToGrowOn: BlockState;
    blocksStalagmiteScan(level: LevelReader, pos: BlockPos, state: BlockState): boolean;
    // private calculateSpeleothemThickness(level: LevelReader, pos: BlockPos, tipDirection: Direction, mergeOpposingTips: boolean): SpeleothemThickness;
    // private calculateTipDirection(level: LevelReader, pos: BlockPos, defaultTipDirection: Direction): Direction;
    canGrow(level: LevelReader, pos: BlockPos): boolean;
    ...
    // private canTipGrow(tipState: BlockState, level: ServerLevel, tipPos: BlockPos): boolean;
    codec(): MapCodec<SpeleothemBlock>;
    createBlockStateDefinition(builder: StateDefinition$Builder<Block, BlockState>): void;
    // private createMergedTips(tipState: BlockState, level: LevelAccessor, tipPos: BlockPos): void;
    // private createSpeleothem(level: LevelAccessor, pos: BlockPos, direction: Direction, thickness: SpeleothemThickness): void;
    getFallDamageSource(entity: Entity): DamageSource;
    getFluidState(state: BlockState): FluidState;
    getMaxGrowthLength(): number;
    ...
    getStalactiteLandingSound(): number;
    getStateForPlacement(context: BlockPlaceContext): BlockState;
    // private grow(level: ServerLevel, growFromPos: BlockPos, growToDirection: Direction): void;
    growStalactiteOrStalagmiteIfPossible(stalactiteStartState: BlockState, level: ServerLevel, stalactiteStartPos: BlockPos, random: RandomSource): void;
    // private growStalagmiteBelow(level: ServerLevel, posAboveStalagmite: BlockPos): void;
    ...
    // private isUnmergedTipWithDirection(state: BlockState, tipDirection: Direction): boolean;
    // private isValidSpeleothemPlacement(level: LevelReader, pos: BlockPos, tipDirection: Direction): boolean;
