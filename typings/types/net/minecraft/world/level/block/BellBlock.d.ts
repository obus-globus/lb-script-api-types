...
export class BellBlock extends BaseEntityBlock {
    ...
    attemptToRing(ringingEntity: Entity, level: Level, pos: BlockPos, direction: Direction): boolean;
    attemptToRing(level: Level, pos: BlockPos, direction: Direction): boolean;
    ...
    // private getVoxelShape(state: BlockState): VoxelShape;
    isPathfindable(state: BlockState, type: PathComputationType): boolean;
    // private isProperHit(state: BlockState, clickedDirection: Direction, clickY: number): boolean;
    ...
    onHit(level: Level, state: BlockState, hitResult: BlockHitResult, player: Player, requireHitFromCorrectSide: boolean): boolean;
