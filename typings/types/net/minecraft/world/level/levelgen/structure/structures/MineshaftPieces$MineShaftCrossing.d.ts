...
export class MineshaftPieces$MineShaftCrossing extends MineshaftPieces$MineShaftPiece {
    ...
    static findCrossing(paramstructurePieceAccessor: StructurePieceAccessor, paramrandom: RandomSource, paramfootX: number, paramfootY: number, paramfootZ: number, paramdirection: Direction): BoundingBox;
    static reorient(paramlevel: BlockGetter, paramblockPos: BlockPos, paramblockState: BlockState): BlockState;
    constructor(genDepth: number, boundingBox: BoundingBox, direction: Direction, type: MineshaftStructure$Type)
    constructor(tag: CompoundTag)
    // private direction: Direction;
    // private isTwoFloored: boolean;
    ...
    // private placeSupportPillar(level: WorldGenLevel, chunkBB: BoundingBox, x: number, y0: number, z: number, y1: number): void;
