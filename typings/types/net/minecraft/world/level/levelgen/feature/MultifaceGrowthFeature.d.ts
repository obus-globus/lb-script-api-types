...
export class MultifaceGrowthFeature extends Feature<MultifaceGrowthConfiguration> {
    ...
    static placeGrowthIfPossible(paramplacerBlock: MultifaceSpreadeableBlock, paramlevel: WorldGenLevel, parampos: BlockPos, paramoldState: BlockState, paramconfig: MultifaceGrowthConfiguration, paramrandom: RandomSource, paramplacementDirections: Direction[]): boolean;
    constructor(codec: Codec<MultifaceGrowthConfiguration>)
    place(context: FeaturePlaceContext<MultifaceGrowthConfiguration>): boolean;
    place(config: MultifaceGrowthConfiguration, level: WorldGenLevel, chunkGenerator: ChunkGenerator, random: RandomSource, origin: BlockPos): boolean;
}