...
export class SpeleothemClusterFeature extends Feature<SpeleothemClusterConfiguration> {
    ...
    constructor(codec: Codec<SpeleothemClusterConfiguration>)
    // private canBeAdjacentToWater(level: LevelAccessor, pos: BlockPos): boolean;
    // private canPlacePool(level: WorldGenLevel, pos: BlockPos, config: SpeleothemClusterConfiguration): boolean;
    // private getChanceOfStalagmiteOrStalactite(xRadius: number, zRadius: number, dx: number, dz: number, config: SpeleothemClusterConfiguration): number;
    // private getSpeleothemHeight(random: RandomSource, dx: number, dz: number, density: number, maxHeight: number, config: SpeleothemClusterConfiguration): number;
    // private isLava(level: LevelReader, pos: BlockPos): boolean;
    place(context: FeaturePlaceContext<SpeleothemClusterConfiguration>): boolean;
    place(config: SpeleothemClusterConfiguration, level: WorldGenLevel, chunkGenerator: ChunkGenerator, random: RandomSource, origin: BlockPos): boolean;
    // private placeColumn(level: WorldGenLevel, random: RandomSource, pos: BlockPos, dx: number, dz: number, chanceOfWater: number, chanceOfStalagmiteOrStalactite: number, clusterHeight: number, density: number, config: SpeleothemClusterConfiguration): void;
    // private replaceBlocksWithBaseBlocks(level: WorldGenLevel, firstPos: BlockPos, maxCount: number, direction: Direction, config: SpeleothemClusterConfiguration): void;
}