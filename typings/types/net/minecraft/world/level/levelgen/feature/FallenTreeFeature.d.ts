...
export class FallenTreeFeature extends Feature<FallenTreeConfiguration> {
    ...
    constructor(codec: Codec<FallenTreeConfiguration>)
    // private canPlaceEntireFallenLog(level: WorldGenLevel, logLength: number, logStartPos: BlockPos$MutableBlockPos, direction: Direction): boolean;
    // private decorateLogs(level: WorldGenLevel, random: RandomSource, logs: BlockPos[], decorators: TreeDecorator[]): void;
    // private getDecorationSetter(level: WorldGenLevel): (param0: BlockPos, param1: BlockState) => void;
    ...
    // private placeStump(config: FallenTreeConfiguration, level: WorldGenLevel, random: RandomSource, stumpPos: BlockPos$MutableBlockPos): void;
    // private setGroundHeightForFallenLogStartPos(level: WorldGenLevel, logStartPos: BlockPos$MutableBlockPos): void;
}