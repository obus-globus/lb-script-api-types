...
export class BasaltColumnsFeature extends Feature<ColumnFeatureConfiguration> {
    ...
    place(context: FeaturePlaceContext<ColumnFeatureConfiguration>): boolean;
    place(config: ColumnFeatureConfiguration, level: WorldGenLevel, chunkGenerator: ChunkGenerator, random: RandomSource, origin: BlockPos): boolean;
    // private placeColumn(level: LevelAccessor, lavaSeaLevel: number, origin: BlockPos, columnHeight: number, reach: number): boolean;
}