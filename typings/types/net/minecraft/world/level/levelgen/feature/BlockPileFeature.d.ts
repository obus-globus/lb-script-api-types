...
export class BlockPileFeature extends Feature<BlockPileConfiguration> {
    ...
    // private mayPlaceOn(level: LevelAccessor, blockPos: BlockPos, random: RandomSource): boolean;
    place(context: FeaturePlaceContext<BlockPileConfiguration>): boolean;
    place(config: BlockPileConfiguration, level: WorldGenLevel, chunkGenerator: ChunkGenerator, random: RandomSource, origin: BlockPos): boolean;
    // private tryPlaceBlock(level: WorldGenLevel, blockPos: BlockPos, random: RandomSource, config: BlockPileConfiguration): void;
}