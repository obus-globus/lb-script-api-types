...
export abstract class AbstractThrownPotion extends ThrowableItemProjectile {
    ...
    static WATER_SENSITIVE_OR_ON_FIRE: (param0: LivingEntity) => kotlin.Boolean;
    ...
    constructor(type: EntityType<AbstractThrownPotion>, level: Level)
    constructor(type: EntityType<AbstractThrownPotion>, level: Level, x: number, y: number, z: number, itemStack: ItemStack)
    constructor(type: EntityType<AbstractThrownPotion>, level: Level, owner: LivingEntity, itemStack: ItemStack)
    calculateHorizontalHurtKnockbackDirection(hurtEntity: LivingEntity, damageSource: DamageSource): DoubleDoubleImmutablePair;
    // private dowseFire(pos: BlockPos): void;
    getDefaultGravity(): number;
    onHit(hitResult: HitResult): void;
    onHitAsPotion(level: ServerLevel, potionItem: ItemStack, hitResult: HitResult): void;
    // private onHitAsWater(level: ServerLevel): void;
