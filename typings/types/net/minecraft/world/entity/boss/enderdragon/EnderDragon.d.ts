...
import type { EnderDragonPhaseManager } from '../../../../../../net/minecraft/world/entity/boss/enderdragon/phases/EnderDragonPhaseManager.d.ts'
...
export class EnderDragon extends Mob implements Enemy {
    ...
    constructor(type: EntityType<EnderDragon>, level: Level)
    body: EnderDragonPart;
    dragonDeathTime: number;
    readonly dragonFight: EnderDragonFight;
    readonly fightOrigin: BlockPos;
    flapTime: number;
    flightHistory: DragonFlightHistory;
    // private growlTime: number;
    head: EnderDragonPart;
    inWall: boolean;
    nearestCrystal: EndCrystal;
    // private neck: EnderDragonPart;
    // private nodeAdjacency: number[];
    // private nodes: Node[];
    oFlapTime: number;
    // private openSet: BinaryHeap;
    readonly phaseManager: EnderDragonPhaseManager;
    // private sittingDamageReceived: number;
    readonly subEntities: EnderDragonPart[];
    // private tail1: EnderDragonPart;
    // private tail2: EnderDragonPart;
    // private tail3: EnderDragonPart;
    // private wing1: EnderDragonPart;
    ...
    checkDespawn(): void;
    // private checkWalls(level: ServerLevel, bb: AABB): boolean;
    defineSynchedData(entityData: SynchedEntityData$Builder): void;
    findClosestNode(): number;
    findClosestNode(tX: number, tY: number, tZ: number): number;
    findPath(startIndex: number, endIndex: number, finalNode: Node): Path;
    getAmbientSound(): SoundEvent;
    getDragonFight(): EnderDragonFight;
    getFightOrigin(): BlockPos;
    getHeadLookVector(a: number): Vec3;
    // private getHeadYOffset(): number;
    ...
    // private knockBack(serverLevel: ServerLevel, entities: Entity[]): void;
    ...
    // private reconstructPath(from: Node, to: Node): Path;
    recreateFromPacket(packet: ClientboundAddEntityPacket): void;
    // private rotWrap(d: number): number;
    sanitizeScale(scale: number): number;
    setDragonFight(fight: EnderDragonFight): void;
    setFightOrigin(fightOrigin: BlockPos): void;
    tickDeath(): void;
    // private tickPart(part: EnderDragonPart, x: number, y: number, z: number): void;
}