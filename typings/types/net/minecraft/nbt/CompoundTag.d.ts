...
export class CompoundTag extends Object implements Tag {
    ...
    read<T extends unknown>(codec: MapCodec<T>, ops: DynamicOps<Tag>): Optional<T>;
    read<T extends unknown>(name: string, codec: Codec<T>): Optional<T>;
    read<T extends unknown>(name: string, codec: Codec<T>, ops: DynamicOps<Tag>): Optional<T>;
    remove(name: string): Tag;
    shallowCopy(): CompoundTag;
    size(): number;
    sizeInBytes(): number;
    store<T extends unknown>(codec: MapCodec<T>, value: T): void;
    store<T extends unknown>(codec: MapCodec<T>, ops: DynamicOps<Tag>, value: T): void;
    store<T extends unknown>(name: string, codec: Codec<T>, value: T): void;
    store<T extends unknown>(name: string, codec: Codec<T>, ops: DynamicOps<Tag>, value: T): void;
    storeNullable<T extends unknown>(name: string, codec: Codec<T>, value: T): void;
    storeNullable<T extends unknown>(name: string, codec: Codec<T>, ops: DynamicOps<Tag>, value: T): void;
    toString(): string;
    values(): Tag[];
