import type { Floor1$InfoFloor1 } from '../../../com/jcraft/jorbis/Floor1$InfoFloor1.d.ts'
import type { Object } from '../../../java/lang/Object.d.ts'
export class Floor1$LookFloor1 extends Object {
    constructor(null_: Floor1$LookFloor1)
    // private forward_index: number[];
    // private frames: number;
    // private hineighbor: number[];
    // private loneighbor: number[];
    // private n: number;
    // private phrasebits: number;
    // private postbits: number;
    // private posts: number;
    // private quant_q: number;
    // private reverse_index: number[];
    // private sorted_index: number[];
    // private vi: Floor1$InfoFloor1;
    free(): void;
}