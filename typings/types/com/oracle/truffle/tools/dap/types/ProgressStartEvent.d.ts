import type { Event } from '../../../../../../com/oracle/truffle/tools/dap/types/Event.d.ts'
import type { ProgressStartEvent$EventBody } from '../../../../../../com/oracle/truffle/tools/dap/types/ProgressStartEvent$EventBody.d.ts'
import type { ProtocolMessage } from '../../../../../../com/oracle/truffle/tools/dap/types/ProtocolMessage.d.ts'
import type { Object } from '../../../../../../java/lang/Object.d.ts'
import type { JSONObject } from '../../../../../../org/graalvm/shadowed/org/json/JSONObject.d.ts'
export class ProgressStartEvent extends Event {
    static create(paramevent: string, paramseq: number): Event;
    static create(parambody: ProgressStartEvent$EventBody, paramseq: number): ProgressStartEvent;
    static create(paramseq: number, paramtype: string): ProtocolMessage;
    constructor(jsonData: JSONObject)
    equals(obj: Object | null): boolean;
    getBody(): ProgressStartEvent$EventBody;
    hashCode(): number;
    setBody(body: ProgressStartEvent$EventBody): ProgressStartEvent;
    setBody(body: Object): Event;
}