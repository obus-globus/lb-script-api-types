...
export class ThreadsHandler extends Object implements ThreadsListener {
    ...
    // private id2threads: { [key: number]: Thread };
    ...
    // private thread2Ids: Map<Thread, number>;
    dispose(): void;
    executeInSuspendedThread(id: number, task: (param0: ThreadsHandler$SuspendedThreadInfo) => boolean): void;
    getThreads(): Thread[];
    onThreadDisposed(ctx: TruffleContext, thread: Thread): void;
    onThreadInitialized(ctx: TruffleContext, thread: Thread): void;
    pause(threadId: number): boolean;
    threadResumed(threadId: number): void;
    threadSuspended(thread: Thread, event: SuspendedEvent): void;
}